...
//  }
  ...
  @Benchmark
  def hashJoin(state: HashIdVsIntIdState): Unit =
    state.db.runNow(state.hashJoin(state.next), requestTimeout)

  @Benchmark
  def intJoin(state: HashIdVsIntIdState): Unit =
    state.db.runNow(state.intJoin(state.next), requestTimeout)
}
