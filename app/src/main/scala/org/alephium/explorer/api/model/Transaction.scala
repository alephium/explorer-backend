...
object TransactionLike {
  implicit val txTemplateRW: ReadWriter[TransactionLike] =
    ReadWriter.merge(ConfirmedTx.txRW, UnconfirmedTx.utxRW)
}

@upickle.implicits.key("confirmed")
final case class ConfirmedTx(
    hash: Transaction.Hash,
    blockHash: BlockEntry.Hash,
    timestamp: TimeStamp,
    inputs: Seq[Input],
...
object ConfirmedTx {
  implicit val txRW: ReadWriter[ConfirmedTx] = macroRW
  def from(tx: Transaction): ConfirmedTx = ConfirmedTx(
    tx.hash,
    tx.blockHash,
    tx.timestamp,
    tx.inputs,
