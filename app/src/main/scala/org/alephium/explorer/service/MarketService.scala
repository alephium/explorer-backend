...
import org.alephium.protocol.model.{Address, ContractId, NetworkId}
...
object MarketService extends StrictLogging {
  ...
  object Impl {
    def default(marketConfig: ExplorerConfig.Market, networkId:NetworkId)(implicit
        ec: ExecutionContext
    ): Impl = {

    val tokenListFile = networkId match {
      case NetworkId.AlephiumMainNet => "mainnet.json"
      case _                         => "testnet.json"
    }

    val tokenListUri =
      uri"https://raw.githubusercontent.com/alephium/token-list/${marketConfig.tokenListRef}/tokens/$tokenListFile"

      new Impl(marketConfig, tokenListUri)
    }
  }

  class Impl(marketConfig: ExplorerConfig.Market, tokenListUri:Uri)(implicit
    ...
    private val ids: ListMap[String, String] = marketConfig.chartSymbolName
     ...
     */
    def baseDelay: Duration = Duration.ofSecondsUnsafe(1)
    ...
    private def tokensToMobulaAssets(tokens: ArraySeq[TokenList.Entry]): ArraySeq[Address] = {
      tokens.map { token =>
    ...
    private def getPricesRemote(retried: Int): Future[Either[String, ArraySeq[Price]]] = {
      tokenListCache.get() match {
        case Right(tokens) =>
          ...
          val assets      = tokensToMobulaAssets(tokens)
    ...
    def convertJsonToMobulaPrices(
      ...
      println(s"${Console.RED}${Console.BOLD}*** json ***${Console.RESET}${json}")
      json match {
        case obj: ujson.Obj =>
            ...
            case Some(data: ujson.Obj) =>
              Try {
                ArraySeq.from(assets.flatMap { asset =>
                  val address = tokensToMobulaAssets(ArraySeq(asset)).head
                  println(s"${Console.RED}${Console.BOLD}*** address ***${Console.RESET}${address}")
                  data.value.get(address.toBase58) match {
                    case Some(value) =>
                      println(s"${Console.RED}${Console.BOLD}*** value ***${Console.RESET}${value}")
                      val price = value("price").num
                      Some(Price(asset.symbol, price))
                    case None =>
                      println(s"${Console.RED}${Console.BOLD}*** None ***${Console.RESET}${None}")
                      None
                  }
                })
