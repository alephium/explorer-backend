...
case object BlockFlowSyncService extends StrictLogging {
  ...
  def start(
      ...
      blockFlowUri: Uri,
      wsFlushInterval: Duration,
      wsMaxBlockDelay: Duration,
      wsMaxBufferSize: Int,
      wsMaxReconnectAttempts: Int,
      wsReconnectBaseDelay: Duration,
      wsReconnectMaxDelay: Duration
  )(implicit
      ec: ExecutionContext,
  ...
  def syncOnce(
    ...
    val syncResult =
      if (initialBackStepDone.get()) {
        ...
        syncOnceWith(nodeUris, blockFlowFetchMaxAge, initialBackStep).map { result =>
    ...
    syncResult.flatMap { isUpToDate =>
      if (isUpToDate) {
        ...
        WebSocketSyncService.sync(
          ...
          // scalastyle:off magic.number
          port = blockFlowUri.port.getOrElse(12973),
          // scalastyle:on magic.number
          flushInterval = wsFlushInterval,
          maxBlockDelay = wsMaxBlockDelay,
