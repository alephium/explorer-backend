...
object QueryUtil {
  ...
  /** Maximum number of parameters allowed by Postgres per query.
    *
    * FIXME: This should be passed as a configuration instead of hardcoding it.
    *        Or it should be fetched from Postgres on boot-up.
    * */
  private val postgresDefaultParameterLimit = 32767
  ...
  def splitUpdates[R](rows: Iterable[R],
                      queryRowParams: Int,
                      queryMaxParams: Int = postgresDefaultParameterLimit)(
  ...
  def splitFoldLeft[R, Q](rows: Iterable[R],
                          initialQuery: Q,
                          queryRowParams: Int,
                          queryMaxParams: Int)(foldLeft: (Iterable[R], String, Q) => Q): Q = {

    //maximum number of rows per query
    val maxRows = Math.floor(queryMaxParams / queryRowParams.toDouble).toInt
