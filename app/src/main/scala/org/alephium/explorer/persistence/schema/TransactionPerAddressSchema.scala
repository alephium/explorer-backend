...
trait TransactionPerAddressSchema extends Schema with CustomTypes {
  ...
  class TransactionPerAddresses(tag: Tag) extends Table[TransactionPerAddressEntity](tag, tableName) {
    ...
    def * : ProvenShape[TransactionPerAddressEntity] =
      (hash, blockHash, timestamp, address, mainChain)
  ...
  val transactionPerAddressesTable: TableQuery[TransactionPerAddresses] = TableQuery[TransactionPerAddresses]
}
