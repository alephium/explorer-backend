...
class InputQueriesSpec extends AlephiumSpec with ScalaFutures {
  ...
  it should "insert and ignore inputs" in new Fixture {
    ...
    def runTest(existingAndUpdated: Seq[(InputEntity, InputEntity)]) = {
      ...
      //insert existing
      runAction(insertInputs(existing)).futureValue
      runAction(InputSchema.table.result).futureValue should contain allElementsOf existing

      //insert should ignore existing inputs
      runAction(insertInputs(ignored)).futureValue is 0
      runAction(InputSchema.table.result).futureValue should contain allElementsOf existing
    }
